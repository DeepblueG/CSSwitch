...
- bridge 只能让系统 OpenSSH 读取真实 `~/.ssh/config`；不得复制或链接整个 `.ssh`，不得暴露私钥内容。为满足 Science 启动前 Host 校验，opt-in 时可在隔离 HOME 写一个 CSSwitch 管理的 `0600` V2 config stub：marker、具体 `Host <aliases...>` 与指向真实 config 的绝对 `Include`。
...
- wrapper 安全结论只能覆盖当前 validator 实际检查的路径无 symlink/目录、普通文件、大小上限和存在 executable bit；没有代码证据时不得宣称已验证 wrapper 内容/hash、owner、nlink、group/world writable 或精确 mode。
- SSH 证据必须拆成 Science parser acceptance、OpenSSH invocation 与真实 server connectivity 三道 gate；后一道不能由前一道推断。
- 特定真实服务器或用户 SSH 配置的连通性需要独立授权和证据。
